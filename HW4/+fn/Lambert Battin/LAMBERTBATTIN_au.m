...
% 
%------------------------------------------------------------------------------  
function [vo, v] = LAMBERTBATTIN_au(rd, ra, dm, Dtsec)
...
r_norm = norm(ra);
rho_norm = norm(rd);
CosDeltaNu= dot(rd,ra)/(rho_norm*r_norm);
rcrossr = cross(rd,ra);
rcrossr_norm = norm(rcrossr);
if strcmp(dm,'pro')
    SinDeltaNu = rcrossr_norm/(rho_norm*r_norm);
else
    SinDeltaNu = -rcrossr_norm/(rho_norm*r_norm);
end
...
tan2w = 0.25*eps*eps / ( sqrt( RoR ) + RoR * ...
                       ( 2.0 + sqrt( RoR ) ) );
rp    = sqrt( rho_norm*r_norm )*( (cos(DNu*0.25))^2 + tan2w );
if ( DNu < pi )
...
x    = 10.0;
xn   = L;
chord= sqrt( rho_norm*rho_norm + r_norm*r_norm - 2.0*rho_norm*r_norm*cos( DNu ) );
s    = ( rho_norm + r_norm + chord )*0.5;
lim1 = sqrt(m/L);
...
if ( a < -small )
    ...
    BetH = 2.0 * asinh( arg2 );
    DH   = AlpH - BetH;
    F    = 1.0 - (a/rho_norm)*(1.0 - cosh(DH) );
    GDot = 1.0 - (a/r_norm) *(1.0 - cosh(DH) );
    ...
    if ( a > small )
        ...
        arg2 = sqrt( ( s-chord ) / ( 2.0*a ) );
        Sinv = arg2;
        Cosv = sqrt( 1.0 - (rho_norm+r_norm-chord)/(4.0D0*a) );
        BetE = 2.0*acos(Cosv);
        BetE = 2.0*asin(Sinv);
        if ( DNu > pi )
            BetE= -BetE;
        ...
        DE  = AlpE - BetE;
        F   = 1.0 - (a/rho_norm)*(1.0 - cos(DE) );
        GDot= 1.0 - (a/r_norm)* (1.0 - cos(DE) );
